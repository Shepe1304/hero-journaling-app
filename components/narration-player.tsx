...
export default function NarrationPlayer({
  ...
  useEffect(() => {
    ...
    const generateAudio = async () => {
      try {
        ...
        // Don't auto-generate audio, wait for user to click play
        // This prevents unnecessary API calls
        setUsingSystemTTS(false);
      } catch {
        console.warn(
          "ElevenLabs preparation failed. Will fall back to system TTS on play."
        );
        // Don't immediately fall back, wait for user interaction
      }
    };
  ...
  const handlePlayPause = async () => {
    ...
    if (!hasPlayedOnce && !audioUrl) {
      // First time playing with ElevenLabs - show loading
      setIsLoading(true);
      ...
      // Audio is already loaded, just play/pause
      if (isPlaying) {
        audioRef.current.pause();
      } else {
        audioRef.current.play();
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <Select
                    ...
                    disabled={usingSystemTTS || isLoading}
                  >
